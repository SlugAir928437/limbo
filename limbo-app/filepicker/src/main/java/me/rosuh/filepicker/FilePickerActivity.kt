...
class FilePickerActivity : AppCompatActivity(), View.OnClickListener,
    ...
    private val loadFileRunnable: Runnable by lazy {
        Runnable {
            ...
            val rootFile = when {
                ...
                navDataSource.isEmpty() && !pickerConfig.isSkipDir && pickerConfig.isFolderPicker -> {
                    // 文件夹选择模式: 需要让根目录也可被选中,取根目录上级作为 rootFiles
                    FileUtils.getRootFile().parentFile
                }

                navDataSource.isEmpty() && !pickerConfig.isSkipDir && !pickerConfig.isFolderPicker -> {
                    // 文件选择模式但允许选文件夹: 直接显示根目录内容
                    FileUtils.getRootFile()
                }
    ...
    private fun initView() {
        ...
        btnSelectedAll?.apply {
            // 文件夹选择模式或单选模式时隐藏
            if (pickerConfig.singleChoice || pickerConfig.isFolderPicker) {
        ...
        btnConfirm?.apply {
            ...
            // 文件夹选择模式下修改按钮文字
            if (pickerConfig.isFolderPicker) {
                text = pickerConfig.folderPickerConfirmText
            } else {
                text = pickerConfig.confirmText
            }
            setOnClickListener(this@FilePickerActivity)
        }
            ...
            R.id.item_list_file_picker -> {
                ...
                if (file.isDirectory) {
                    ...
                    if (pickerConfig.singleChoice) {
                        ...
                        listAdapter.multipleCheckOrNo(item as FileItemBeanImpl, position, ::isCanSelect) {
                            Toast.makeText(
    ...
    override fun onClick(v: View?) {
        when (v!!.id) {
            ...
            R.id.btn_confirm_file_picker -> {
                if (pickerConfig.isFolderPicker) {
                    // 文件夹选择模式:
                    // 优先返回已勾选的文件夹; 未勾选时回退到当前目录
                    val checkedItems = listAdapter.dataList?.filter { it.isChecked() }?.map { it.filePath }
                    if (!checkedItems.isNullOrEmpty()) {
                        FilePickerManager.saveData(checkedItems.toMutableList())
                    } else {
                        val currDir = navDataSource.lastOrNull()?.dirPath
                        if (currDir.isNullOrBlank()) {
                            setResult(Activity.RESULT_CANCELED)
                            finish()
                            return
                        }
                        FilePickerManager.saveCurrDir(currDir)
                    }
