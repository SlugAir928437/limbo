...
public class Machine extends Observable {
    ...
    // Accelerator modes, persisted in ACCEL_MODE (-accel <name>)
    public static final String ACCEL_TCG = "tcg";
    public static final String ACCEL_KVM = "kvm";
    ...
    private String accelMode = ACCEL_TCG;
    ...
    public String getAccelMode() {
        return accelMode;
    }

    void setAccelMode(String accelMode) {
        if (accelMode == null)
            accelMode = ACCEL_TCG;
        if (this.accelMode == null || !this.accelMode.equals(accelMode)) {
            this.accelMode = accelMode;
            setChanged();
            notifyChanged(MachineProperty.ACCEL_MODE, accelMode);
        }
    }
