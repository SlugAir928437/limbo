...
class FilePickerConfig(private val pickerManager: FilePickerManager) {
    ...
    /**
     * 是否为文件夹选择模式。
     * 在此模式下,用户浏览到目标文件夹后点击确认按钮,当前目录路径即被返回。
     */
    var isFolderPicker = false
        private set
     ...
     * 文件夹选择模式下确认按钮文字
     */
    var folderPickerConfirmText: String = contextRes.getString(R.string.file_picker_tv_select_folder)
        private set
    ...
    /**
     * 启用文件夹选择模式。
     * 在此模式下,用户浏览到目标文件夹后点击确认按钮即可选中该文件夹。
     * 同时会自动设置 [isSkipDir] 为 false 以允许文件夹被选中。
     */
    fun folderPicker(): FilePickerConfig {
        isFolderPicker = true
        isSkipDir = false
        return this
    }
